...
public class QuizActivity extends AppCompatActivity {
    ...
    final QuestionFetcher.OnQuestionReceivedListener mFetcherQuestionListener = new QuestionFetcher.OnQuestionReceivedListener() {
        @Override
        public void onQuestionReceived(Question question) {
            mQuestion = question;
            displayTextView.setText(Html.fromHtml(mQuestion.questionText));
            // take the special char  that is in html into text
        }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        updateScore();
        //todo add  the quesiton thifn 10 min into class
        mQuestionFetcher = new QuestionFetcher(this);
        mQuestionFetcher.fetchQuestion(mFetcherQuestionListener);
    ...
    private void checkAnswer(Boolean userPress){
        if(userPress.equals(mQuestion.answerTrue)){
            answerTextView.setTextColor(getResources().getColor(R.color.purple_700));
    ...
    private class NextButtonClickListener implements View.OnClickListener{
        ...
        public void onClick(View v) {
            ...
            mQuestionFetcher.fetchQuestion(mq); //FIXME soem thign here 12min in class
            //fetch next question
        }
    }
}